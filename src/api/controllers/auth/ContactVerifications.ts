...
export const verifyCode = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    if (!mobileNumber || !verificationCode) {
      return res
        .status(400)
        .json({ status: 'error', message: 'Please provide both mobile number and verification code.' });
    }
    console.log(mobileNumber, verificationCode);

    const isVerify = await ContactVerification.findOne({ where: { mobileNumber, verificationCode } });
