...
export const flaggedWords = [
    ...
    'allahu akbar', 'Abbo', 'abo', 'abortion', 'abuse', 'addict', 'addicts', 'adult', 'alla', 'allah', 'alligatorbait',
...
export const checkExplicitText = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (content) {
            ...
            if (hasFlaggedWord) {
                return res.status(400).json({ status: "fail", message: "Inapporpriate text detected" });
            }
        ...
        console.error("Error in text moderation:", error);
        res.status(500).json({ status: "error", message: "Error checking text content" });
    }
};
